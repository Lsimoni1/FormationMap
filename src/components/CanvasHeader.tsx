import { ArrowUpLeft, ChevronLeft, ChevronRight, Plus, AlignJustify} from 'lucide-react'
import { Button } from '@/components/ui/button'
import { useNavigate } from 'react-router-dom'

interface CanvasHeaderProps {
    projectTitle: string
    onPrev: () => void
    onNext: () => void
    onSave: () => void
    onMenuToggle: () => void
}

const CanvasHeader = ({projectTitle, onPrev, onNext, onSave, onMenuToggle}: CanvasHeaderProps) => {
    const navigate = useNavigate();

    return(
        <div className="flex items-center justify-between px-4 py-2 border-b-2 border-gray-300"> {/* container for header */}

            <div className="flex gap-4 "> {/* container for back button and title*/}
                <Button /* back button */
                title="Return to Project Dashboard"
                onClick={() => navigate('/dashboard')}
                variant="outline"
                size="icon-lg"
                >
                    <ArrowUpLeft />
                </Button>

                <h1 className="scroll-m-20 text-3xl font-bold tracking-tight">
                    {projectTitle}
                </h1>
            </div>

            <div className="flex gap-6"> {/* container for prev, next, save, and formation menu toggle buttons */}
                <Button /* previous formation button */
                title="Switch to previous formation"
                onClick={onPrev}
                variant="outline"
                size="icon-lg"
                >
                    <ChevronLeft />
                </Button>

                <Button /* next formation button */
                title="Switch to next formation"
                onClick={onNext}
                variant="outline"
                size="icon-lg"
                >
                    <ChevronRight />
                </Button>

                <Button /* save new formation button */
                title="Save canvas state as next formation"
                onClick={onSave}
                variant="outline"
                size="icon-lg"
                >
                    <Plus />
                </Button>

                <Button /* toggle formations menu button */
                title="Toggle Formations Menu"
                onClick={onMenuToggle}
                variant="outline"
                size="icon-lg"
                >
                    <AlignJustify />
                </Button>
            </div>
            
           
        </div>
    );
};

export default CanvasHeader;